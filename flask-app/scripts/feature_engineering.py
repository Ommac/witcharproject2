...
class FeatureEngineering:
    ...
    def create_features(self):
        ...
        # For inference: first row per user has NaN from shift(1); treat as no change
        self.df['ip_location_change'] = self.df['ip_location_change'].fillna(False)
        self.df['source_change'] = self.df['source_change'].fillna(False)
        self.df['browser_change'] = self.df['browser_change'].fillna(False)
    ...
    def scale_features(self):
        ...
        # Fill NaN for inference (e.g. single-row or unseen categories)
        self.df[numeric_columns] = self.df[numeric_columns].fillna(0)
        # Scale the numeric features (avoid 0 variance -> NaN: use scale only when std > 0)
        scaled = self.scaler.fit_transform(self.df[numeric_columns])
        self.df[numeric_columns] = np.where(np.isfinite(scaled), scaled, 0)
